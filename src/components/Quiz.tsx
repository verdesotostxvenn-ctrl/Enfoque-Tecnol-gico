...
interface Question {
  pregunta: string;
  opciones: string[];
  correcta: number;import React, { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { BrainCircuit, Star, AlertTriangle, CheckCircle2, XCircle, Loader2 } from 'lucide-react';
import { supabase } from '../supabaseClient'; 
...
const Quiz: React.FC<QuizProps> = ({ tipo, onWin, onClose }) => {
  ...
  const handleLevelUp = async () => {
    ...
    try {
      if (nombreAgente) {
        ...
        // Usamos .toUpperCase() para asegurar que la búsqueda sea exacta si hubo cambios de case
      ...
      // Pequeña pausa dramática para que el niño sienta la importancia del proceso
      setTimeout(() => {
        onWin(); 
      }, 1500);
      ...
      // Aun si falla la red, permitimos que avance para no frustrar al niño, 
      // el localStorage ya debería estar listo para el Hub.
      onWin();
    } finally {
      setIsSyncing(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      ...
      >
        {/* Nube de pensamiento - Puntero táctico */}
        ...
        <div className="absolute -bottom-5 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[16px] border-l-transparent border-r-[16px] border-r-transparent border-t-[20px] border-t-[#010413]"></div>
        ...
        {isSyncing ? (
          <div className="py-20 flex flex-col items-center justify-center space-y-6">
            ...
            <h2 className="text-xl font-black text-white uppercase tracking-widest text-center animate-pulse">
