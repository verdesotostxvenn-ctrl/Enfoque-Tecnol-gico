...
const getThreatColor = (value?: number) => {
  return susceptibilityPalettes.institucional.find((item) => item.value === value)?.color || '#ef4444';
};
    ...
    const cargarMapaPublicado = async () => {
      ...
      try {
        ...
        if (mapa.storage_folder) {
          ...
          try {
            setEstado('Cargando raster procesado con puntos de instituciones...');
        ...
        <header className="rounded-[2rem] border border-white/10 bg-white/5 p-5 md:p-7 backdrop-blur-2xl shadow-[0_30px_100px_rgba(0,0,0,0.35)]">
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_0.85fr] lg:items-end">
            ...
            <div className="rounded-[1.7rem] border border-cyan-300/20 bg-cyan-400/10 p-4 flex items-start gap-3">
              ...
              <p className="text-sm text-cyan-50/80 font-semibold leading-relaxed">Pasa el mouse sobre el mapa para identificar el nivel de amenaza. Activa instituciones para ver escuelas y su amenaza.</p>
            </div>
          </div>
        </header>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-5">
          <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-4 md:p-5 shadow-2xl">
            ...
            <div
              ...
              onPointerLeave={() => { setDragging(false); setHoverInfo(null); setActivePoint(null); }}
              onPointerCancel={() => { setDragging(false); setHoverInfo(null); setActivePoint(null); }}
            ...
            >
              ...
              >
                <img ref={imageRef} src={mapaUrl} alt="Mapa de amenaza por inundaciones" draggable={false} className="h-full w-full object-contain" />
                {showSchools && schoolPoints.map((point) => (
                  <button
                    key={point.id}
                    type="button"
                    onPointerEnter={() => setActivePoint(point)}
                    onPointerLeave={() => setActivePoint(null)}
                    onClick={(event) => { event.stopPropagation(); setActivePoint(point); }}
                    className="absolute z-10 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-[0_0_18px_rgba(239,68,68,0.75)] transition-transform hover:scale-150"
                    style={{ left: `${(point.xRatio || 0) * 100}%`, top: `${(point.yRatio || 0) * 100}%`, backgroundColor: getThreatColor(point.threatValue) }}
                    title={point.name}
                  />
                ))}
              </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 backdrop-blur-2xl">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(Object.keys(susceptibilityPalettes) as PaletteName[]).map((palette) => <button key={palette} onClick={() => setActivePalette(palette)} disabled={!raster} className={`rounded-2xl border px-3 py-2 text-xs font-black uppercase tracking-[0.12em] transition-all ${activePalette === palette ? 'border-cyan-300 bg-cyan-400 text-slate-950' : 'border-white/10 bg-slate-950/55 text-slate-300 hover:border-cyan-300/50'} disabled:opacity-50 disabled:cursor-not-allowed`}>{paletteLabels[palette]}</button>)}
              </div>
            </div>

            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 backdrop-blur-2xl">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3"><MapPin className="text-orange-300" size={22} /><div><p className="text-orange-300 text-[10px] font-black uppercase tracking-[0.3em]">Instituciones</p><h2 className="text-2xl font-black">Escuelas</h2></div></div>
                <button onClick={() => setShowSchools((prev) => !prev)} disabled={!schoolPoints.length} className="rounded-2xl border border-white/10 bg-slate-950/60 p-3 text-cyan-200 hover:bg-cyan-400 hover:text-slate-950 disabled:opacity-40">
            ...
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-emerald-300/20 bg-emerald-400/10 p-5">
              <p className="text-emerald-300 text-[10px] font-black uppercase tracking-[0.28em] mb-2">Uso educativo</p><h3 className="text-xl font-black">¿Para qué sirve?</h3><p className="mt-3 text-slate-300 text-sm font-semibold leading-relaxed">Ayuda a identificar zonas con mayor amenaza de inundación y relacionarlas con instituciones educativas, rutas seguras y puntos de encuentro.</p>{updatedAt && <p className="mt-4 text-xs font-bold text-slate-500">Última actualización: {new Date(updatedAt).toLocaleString('es-EC')}</p>}
            </motion.div>
          </aside>
        </section>
      </section>
