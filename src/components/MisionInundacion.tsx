...
const MisionInundacion = () => {
  ...
  const handleWinQuiz = async () => {
    ...
    const { error } = await supabase
      .from('agentes') 
      .update({ mision_inundacion: true, ultima_conexion: new Date() })
      .eq('nombre', nombre);

    if (!error) {
      setIsCompleted(true);
      localStorage.setItem('agenteNivel', '3'); // 👈 ¡Sube a Nivel 3!
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white p-6 md:p-12 relative overflow-hidden cursor-none">
      ...
      >
        <ChevronLeft size={20} /> <span className="text-xs font-black uppercase tracking-widest text-white ml-2">Volver al Hub</span>
      </button>
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-slate-900/50 p-8 rounded-3xl border border-blue-500/20 backdrop-blur-md">
            ...
            <p className="text-slate-400 text-sm">Si el agua entra en casa, corta la luz de inmediato para evitar cortocircuitos.</p>
          </div>
          <div className="bg-slate-900/50 p-8 rounded-3xl border border-blue-500/20 backdrop-blur-md">
            <MapPin className="text-blue-500 mb-4" size={32} />
            <h3 className="text-xl font-bold mb-2 text-white">Zonas Altas</h3>
            <p className="text-slate-400 text-sm">Ubica el punto más alto de tu escuela o comunidad y dirígete allí sin correr.</p>
          </div>
        </div>

        {/* 👈 BOTÓN PARA ABRIR LA TRIVIA */}
        <button 
        ...
        >
          {loading ? 'PROCESANDO...' : isCompleted ? (
            <>
