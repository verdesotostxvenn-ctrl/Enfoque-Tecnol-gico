...
const Hub = () => {
  ...
  const sincronizarDatos = useCallback(() => {
    ...
    if (!Number.isNaN(nivelNumerico) && nivelNumerico >= 1) {
      setNivelAgente(Math.min(nivelNumerico, 4));
    } else {
      setNivelAgente(1);
  ...
  useEffect(() => {
    ...
    const handleCustomSync = () => sincronizarDatos();

    window.addEventListener('mousemove', handleMouseMove);
    window.addEventListener('focus', handleFocus);
    window.addEventListener('storage', handleStorageChange);
    window.addEventListener('agenteNivelActualizado', handleCustomSync);
    ...
    const intervaloSync = window.setInterval(() => {
      if (!document.hidden) sincronizarDatos();
    }, 1000);
    ...
    return () => {
      ...
      window.removeEventListener('agenteNivelActualizado', handleCustomSync);
    ...
    <div className="min-h-screen w-full bg-[#010413] text-white p-6 md:p-16 relative overflow-hidden cursor-none">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute bottom-0 right-0 w-96 h-96 bg-orange-500/20 rounded-full blur-[120px]"
        />
      </div>
      ...
      <header className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6 bg-white/5 p-8 rounded-[2.5rem] border border-white/10 backdrop-blur-md">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <div className="flex items-center space-x-3 text-cyan-400 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em]">
              Terminal Agente Distrito 18D03
            </span>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end space-y-4 md:space-y-0 md:space-x-4">
            ...
            <div className="flex flex-wrap gap-3">
              <motion.div
                ...
                className="flex items-center space-x-2 bg-orange-500/10 border border-orange-500/30 px-4 py-2 rounded-xl text-orange-400 text-[10px] font-black uppercase tracking-widest shadow-[0_0_15px_rgba(249,115,22,0.2)]"
              >
                <Trophy size={14} />
              ...
              <motion.div
                ...
                animate={{ scale: 1, opacity: 1 }}
                className="flex items-center space-x-2 bg-cyan-500/10 border border-cyan-500/30 px-4 py-2 rounded-xl text-cyan-300 text-[10px] font-black uppercase tracking-widest"
              >
                <RefreshCw size={14} />
        ...
        <button
          ...
          onMouseEnter={() => setIsHovering(true)}
          onMouseLeave={() => setIsHovering(false)}
          className="flex items-center space-x-3 bg-red-500/10 border border-red-500/30 px-6 py-3 rounded-2xl text-red-500 text-[10px] font-black uppercase hover:bg-red-500 hover:text-black transition-all"
        >
          <LogOut size={16} />
        ...
        {misiones.map((mision, index) => {
          ...
          return (
            ...
            >
              ...
              <h2 className="text-2xl font-black mb-4 uppercase tracking-tighter text-white">
                {mision.titulo}
              </h2>

              <p className="text-white/70 text-xs leading-relaxed font-medium mb-6 uppercase tracking-wide">
                {mision.desc}
              </p>
      ...
      {nivelAgente < 4 && (
        <div className="relative z-10 mt-10 bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-md">
          ...
          <p className="text-xs text-white/60 uppercase tracking-wider">
            Completa las misiones activas para subir de nivel. Al alcanzar el Nivel 4, se desbloqueará la Credencial de Comandante.
          </p>
        </div>
      )}
            ...
            >
              <div className="relative bg-slate-900/95 h-full rounded-[3.4rem] p-10 flex flex-col items-center overflow-hidden">
                ...
                <div className="relative w-48 h-48 bg-gradient-to-tr from-yellow-900/40 to-amber-900/40 rounded-[3rem] border-2 border-yellow-500/40 flex items-center justify-center mb-8 shadow-inner">
                  ...
                  <div className="absolute -bottom-4 bg-yellow-500 text-black text-[12px] font-black px-6 py-2 rounded-full uppercase shadow-xl border-2 border-slate-900">
                    COMANDANTE ÉLITE
                  </div>
                </div>
                ...
                <div className="w-full flex items-center justify-between bg-black/40 p-4 rounded-2xl border border-white/5">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[8px] text-yellow-500/40 font-mono uppercase mt-1 tracking-tighter">
                      Sincronizado con Distrito 18D03
                    </p>
                  </div>
                </div>
      ...
      <footer className="relative z-10 mt-16 flex flex-col md:flex-row gap-4 justify-between items-center opacity-40">
        <div className="flex items-center space-x-4">
          <Database size={14} className="text-cyan-500" />
          <span className="text-[9px] font-bold uppercase tracking-widest text-white">
            Sincronización Satelital: OK
          </span>
        </div>
        <span className="text-[9px] font-bold text-white uppercase tracking-widest">
          ESPOCH - INVESTIGACIÓN RESILIENCIA 2026
        </span>
      </footer>
    </div>
  );
