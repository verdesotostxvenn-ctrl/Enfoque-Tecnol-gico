...
const Lobby = () => {
  ...
  // Posición del cursor con actualización inmediata
  const [mousePos, setMousePos] = useState({ x: -100, y: -100 });

  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      // Usamos setMousePos para un seguimiento directo y sin suavizado
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 md:p-10 relative overflow-hidden bg-[#020617]">
      ...
      >
        <div className="w-6 h-6 border-2 border-cyan-400 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(34,211,238,0.6)]">
          <div className="w-1 h-1 bg-white rounded-full" />
        </div>
      </div>

      {/* 🔵 EFECTO BOKEH INTENSIFICADO (Ahora sí se verá) */}
      <div className="absolute inset-0 pointer-events-none z-0">
        <motion.div 
          ...
          className="absolute top-0 left-0 w-[600px] h-[600px] bg-cyan-500/10 rounded-full blur-[100px]"
        />
        <motion.div 
          animate={{ 
            x: [0, -120, 120, 0], 
          ...
          className="absolute bottom-0 right-0 w-[700px] h-[700px] bg-emerald-500/10 rounded-full blur-[130px]"
        />
      </div>
      ...
      <motion.div 
        ...
        className="relative w-full max-w-6xl bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-white/5 shadow-2xl flex flex-col lg:flex-row overflow-hidden z-10"
      >
        ...
        <div className="w-full lg:w-1/2 p-10 md:p-16 border-b lg:border-b-0 lg:border-r border-white/5">
          <div className="flex items-center space-x-3 text-cyan-400 mb-10">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em]">Protocolo Activo</span>
          </div>
          ...
          <p className="text-slate-400 text-sm md:text-base leading-relaxed max-w-md mb-12">
            Gestión estratégica de riesgos naturales para el <span className="text-white font-bold underline decoration-cyan-500/40">Distrito 18D03</span>.
          </p>

          <div className="flex items-center space-x-4 opacity-30">
            <Database size={20} />
            <div className="h-[1px] w-12 bg-white/10" />
            <span className="text-[10px] font-bold uppercase tracking-widest">v1.2.7</span>
          </div>
        </div>
            ...
            >
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-slate-950/50">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-xs font-black uppercase tracking-[0.3em]">Unidades del Distrito</h2>
                </div>
              ...
              <div className="p-8 max-h-[60vh] overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-3 custom-scrollbar">
                {escuelasDisponibles.map((item) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black tracking-tight uppercase">{item}</span>
                  </button>
                ))}
              </div>
            </motion.div>
